...
def get_pressure_force(exo_file, uinf, iR, curR, Nrev, NIt_per_rev, steps):
    ...
    print(np.size(tsteps))
    print(tsteps[0]) 
    print(tsteps[-Nend])
#    renderView1.ViewTime = tsteps[-1]
    renderView1.ViewTime = tsteps[-Nend]
    bshow = Show(calculator2, renderView1)

    # ******* Compute Pressure Forces *******
    tstep_range = range(steps)
    curT_vec = tsteps[-steps-Nend:-Nend]
#    tstep_range = range(curT_vec)
 
    print(np.size(curT_vec)) 
...
###############################################
# ***************** Plots *********************
def compare_cth_exp_data(C_th_avg, uinf):
    ...
    with PdfPages('C_th_comp_' + str(uinf) + '.pdf') as pfpgs:
         plt.figure()
         plt.scatter(rbyR, cth_avg_exp_data, marker='o', color='black', label='Exp - Avg')
         plt.scatter(rbyR, cth_min_exp_data, marker='o', color='blue', label='Exp - Min.')
         plt.scatter(rbyR, cth_max_exp_data, marker='o', color='red', label='Exp - Max.')
         plt.scatter(rbyR, C_th_avg, marker='x', color='black', label='IDDES')
         plt.vlines(rbyR[0], cth_min_exp_data[0], cth_max_exp_data[0])
         plt.vlines(rbyR[1], cth_min_exp_data[1], cth_max_exp_data[1])
         plt.vlines(rbyR[2], cth_min_exp_data[2], cth_max_exp_data[2])
         plt.vlines(rbyR[3], cth_min_exp_data[3], cth_max_exp_data[3])
...
def compare_ctq_exp_data(C_tq_avg, uinf):
    ...
    with PdfPages('C_tq_comp_' + str(uinf) + '.pdf') as pfpgs:
         plt.figure()
         plt.scatter(rbyR, ctq_avg_exp_data, marker='o', color='black', label='Exp - Avg')
         plt.scatter(rbyR, ctq_min_exp_data, marker='o', color='blue', label='Exp - Min.')
         plt.scatter(rbyR, ctq_max_exp_data, marker='o', color='red', label='Exp - Max.')
         plt.scatter(rbyR, C_tq_avg, marker='x', color='black', label='IDDES')
         plt.vlines(rbyR[0], ctq_min_exp_data[0], ctq_max_exp_data[0])
         plt.vlines(rbyR[1], ctq_min_exp_data[1], ctq_max_exp_data[1])
         plt.vlines(rbyR[2], ctq_min_exp_data[2], ctq_max_exp_data[2])
         plt.vlines(rbyR[3], ctq_min_exp_data[3], ctq_max_exp_data[3])
         plt.legend(loc='upper right') 
         plt.xlabel('r/R')
...
if __name__=="__main__":
    ...
    # Plots:
    compare_cth_exp_data(C_th_avg, uinf)
    compare_ctq_exp_data(C_tq_avg, uinf)
